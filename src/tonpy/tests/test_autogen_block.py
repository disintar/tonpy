...
def test_block_autogen_unpack():
    ...
    # Load augmented typed dictionary with recursively unpack all fields with autogenerated TLB
    in_msgs = TypedVmDict((InMsg(), ImportFees()),
                          256, False,
                          in_msg_descr, rec_unpack=True)

    # You can get all messages
    # for i in in_msgs:
    #     print(i)

    # Get one value for test
    test_msg = in_msgs[55320535438392264092286370946255453957461499360282925697254340001519613742160]
    # You can access extra value
    assert test_msg.extra.fees_collected.amount.value == 666672
    assert test_msg.extra.value_imported.grams.amount.value == 100666672

    # And any value recursively unpacked by TLB
    assert int(test_msg.data.transaction.account_addr, 2) == \
           0x86e9617280e747235574d07d7b0202cf5a2b3c6d70f583a82dda3e8bb9b1b0c6
    assert test_msg.data.transaction.now == 1612979175
    assert test_msg.data.transaction.lt == 15441606000010
    assert test_msg.data.transaction.orig_status == 2
    assert test_msg.data.transaction.end_status == 2
    assert test_msg.data.transaction.outmsg_cnt == 0

    # Test very deep value
    assert test_msg.data.transaction.description.compute_ph.r1.gas_used.value == 409
